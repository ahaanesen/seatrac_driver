...
mod seatrac;
mod comms;
use crate::{comms::{ack_manager::{self, SentMsgManager}, ros2_node}, modem_driver::ModemDriver};
    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            if tdma_scheduler.is_my_slot() {
                ...
                let acks_to_send = ack_handler.initialize_ack_slot();
                tdma_scheduler.broadcast_status_msg(&mut modem, initial_time as u64, &mut ack_handler, &acks_to_send, comms_config.propagation_time); // TODO: remove ack_handler from here?
                ...
                while tdma_scheduler.is_my_slot() {
                    ...
                    if  !ack_handler.is_empty() && next_slot == tdma_scheduler.assigned_slot {
                        tdma_scheduler.resend_messages(&mut modem, &mut ack_handler, &acks_to_send, comms_config.propagation_time);
                    }
